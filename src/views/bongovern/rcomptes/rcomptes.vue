<template>
            ...
            <el-table-column prop="value" label="Value">
              <template slot-scope="{row}">
                <div v-if="row.field === 'Title'">
                  ...
                  ></el-input>
                  <span v-else>{{ editingPost.acf.titulo_est }}</span>
                  <el-button
                    v-if="isEditingTitle"
                    size="mini"
                ...
                <div v-if="row.field === 'Description'">
                  ...
                  <span v-else>{{ editingPost.acf.descripcion_infografia_rcomptes }}</span>
                  <el-button
                    v-if="isEditingDescription"
                    size="mini"
  ...
  export default {
   ...
   /* watch: {
    ...
    data() {
      ...
      postType: 'estadisticas', // Mantén solo una definición de 'postType'
      listQuery: {
          titulo_est: '',
          importance: '',
          type: '',
    ...
    async fetchPosts(query = {}, postType = 'estadisticas', page = 1, per_page = 10, acfFilterValue = '', acfDropdownValue = '') {
          ...
          'page': page,
          'per_page': per_page,
          'filter[meta_key]': 'titulo_est',
          'filter[meta_value]': query.titulo_est,
    ...
    /*
    async handleFilter() {
          await this.fetchPost(this.listQuery);
      },*/
      async handleFilter() {
        console.log("Post type:", this.postType);
      await this.fetchPosts(this.postType, this.currentPage, this.pageSize, this.listQuery.titulo_est);
  },
  ...
  async fetchAndSetPosts() {
     ...
     const response = await fetchPosts(this.postType, this.currentPage, this.pageSize, this.listQuery.titulo_est);
  ...
  async saveFieldEdit(fieldName, fieldValue) {
          ...
          if (fieldName === 'Image') {
              updatedData.imatge_rcomptes = this.editableImage;
          } else {
              const fieldMap = {
                  'Description': 'descripcion_infografia_rcomptes',
                  'Title': 'titulo_est',
          ...
          try {
             ...
             await updatePost('estadisticas', this.editingPost.id, updatedData);
  ...
  */
       handleEdit(post) {
        ...
        this.editableTitle = post.acf.titulo_est; // Añadir esta línea
        this.editableDescription = post.acf.descripcion_infografia_rcomptes; // Añade esta línea
        this.editableImage = post.acf.imatge_rcomptes;
      ...
      // this.editableCategoriaOds = post.acf.categorias_ods;
        ...
        this.editFields = [
          { field: 'Title', value: post.acf.titulo_est },
          { field: 'Date', value: post.date },
          //{ field: 'Description', value: post.content.rendered },
          { field: 'Description', value: post.acf.descripcion_infografia_rcomptes },
          ...
          { field: 'Image', value: post.acf.imatge_rcomptes },
